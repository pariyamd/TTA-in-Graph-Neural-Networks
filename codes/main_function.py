...
from model.model_utilies import CoraData
import os
import shutil
...
def main(params):
    ...
    parser.add_argument('--adaptation_method', type=str, default="soga",
                        help='homottt')
    ...
    args.source_dataset = "ACM"
    args.target_dataset = "DBLP"
    print(args.source_dataset, args.target_dataset)
    num_nodes_dict = {"DBLP": 5578, "ACM": 7410, "acm_a_1": 1500, "acm_b_1": 1500, "Cora": 2708}
    num_label_dict = {"DBLP": 6, "ACM": 6, "acm_a_1": 4, "acm_b_1": 4,  "Cora": 7}
    num_feature_dict = {"DBLP": 7537, "ACM": 7537, "acm_a_1": 300, "acm_b_1": 300, "Cora": 1433}
    ...
    if args.source_dataset == "Cora":
        dataset = CoraData(root=cora_dir)
        source_data = dataset[0].to(device)
    elif args.source_dataset in dataset_list:
    ...
    if "Cora" in args.target_dataset:
        processed_dir = os.path.join(cora_dir, 'processed')
        vars(args)['num_target_nodes'] = num_nodes_dict[args.source_dataset]
        if os.path.exists(processed_dir):
            shutil.rmtree(processed_dir)
        dataset_noisy = CoraData(root=cora_dir, noise_ratio=0.05, noise_type='flip',valid_ratio=0.1)
        target_data = dataset_noisy[0].to(device)

    elif args.target_dataset in dataset_list:
    ...
    micro,macro = train_procedure(args, logger, model, source_optimizer, target_optimizer, criterion, source_data, target_data, target_structure_data, structure_adj)
